...
app.route("/employees/all").get(async (req, res) => {
  const results = await dbClient
...
app.route("/employees/all/:name").get(async (req, res) => {
  const { name } = req.params;

  const results = await dbClient
    .query("SELECT * FROM employees WHERE name ILIKE '%' || $1 || '%'", [name])
    .then((payload) => {
      return payload.rows;
    })
    .catch(() => {
...
app
  ...
  .delete(async (req, res) => {
    ...
    try {
      await dbClient.query(
        "UPDATE employees set title='inactive' WHERE id=$1;",
        [id]
      );
