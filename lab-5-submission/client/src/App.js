...
import { getDataQuery, set, updateData } from "./components/dataStoreLocal";
  ...
  const fetchEmployees = async () => {
    try {
      if (isOnline) {
        ...
        const offlineData = await getDataQuery("employees", "");
  ...
  const findRecord = async () => {
    try {
      if (isOnline) {
        ...
        const filteredData = offlineData.filter((employee) =>
          employee.name.toLowerCase().includes(lowercaseNameToFind)
        );
